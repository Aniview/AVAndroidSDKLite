...
import com.aniview.sdk.player.bridge.ComplexPlayerBridge
...
class ComplexExoActivity : AppCompatActivity() {

	private val mDisposable = CompositeDisposable()
	private var mBridge: ComplexPlayerBridge? = null
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		val generalInfo = findViewById<TextView>(R.id.general_info)
		val debugInfo = findViewById<TextView>(R.id.debug_info)
		...
		val contentPlayer = ExoPlayerFactory.newSimpleInstance(this, DefaultTrackSelector())
		val contentExt = ExoPlayerExtension(this, contentPlayer)

		//this is used to share surface between players - switching is done by the SDK extension
		contentExt.setSurface((playerView?.videoSurfaceView as? SurfaceView)?.holder?.surface)

		val adPlayer = ExoPlayerFactory.newSimpleInstance(this, DefaultTrackSelector())
		val adExt = ExoPlayerExtension(this, adPlayer)
		...
		val bridge = ComplexPlayerBridge(container, contentExt, adExt)
		bridge.addAdCallback(object : PlayerBridge.Callback() {
			override fun onPlay() {
				//this is used for the Exo Player controls to subscribe to the current player
				playerView.player = adPlayer
			}
		})
